...
scalaio.controller('SpeakerCtrl', ['$scope', '$routeParams', '$sce', 'Speakers', function($scope, $routeParams, $sce, Speakers) {
    Speakers.get({ speakerId: $routeParams.speakerId },
        function(data) {
            $scope.speaker = data;
            $scope.speakerBio = $sce.trustAsHtml('<img class="avatar left" src="' + data.avatar + '"></img>' + data.bio);
        });
...
scalaio.controller('SubmissionCtrl', function($scope) {});
scalaio.controller('SubmissionsCtrl', function($scope) {});